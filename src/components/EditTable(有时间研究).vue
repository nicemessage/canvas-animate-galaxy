  ...
  <div class="el-table-container">
    <div>
      <div style="padding: 8px 0 8px 0;float: right;margin-top: -40px">
        <el-upload v-if="showImportButton"
        ...
        <el-table-column v-for="column in columns"
          ...
          <template slot-scope="scope">
            <span style="width:100%;"
                  v-if="column.type=='select'&& (column.dataSource ==undefined || column.dataSource!='dynamic')"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                ...
                <el-option v-for="(item,index) in column.source"
                           :key="index"
                           :value="item[column.props.value]"
                           :label="item[column.props.label]"></el-option>
              </el-select>
            </span>
            <span v-else-if="column.type=='select'&& column.dataSource && column.dataSource=='dynamic'"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                         size="small"
                         :class="validate1(scope.row[column.prop],column.rule,scope.row)?'':'has-error'"
                         :disabled="column.disabled==undefined?false:column.disabled">
                <el-option-group v-for="group in column.source(scope.row)"
                                 :key="group.label"
                                 :label="group.label">
            ...
            <span v-else-if="column.type=='select_dynamic'"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                         size="small"
                         style="width:100%"
            ...
            <span v-else-if="column.type=='select_edit_no_create'"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                         size="small"
                         filterable
                         style="width:100%"
                         :class="validate1(scope.row[column.prop],column.rule,scope.row)?'':'has-error'"
                         :placeholder="column.placeholder == undefined ? '请选择':column.placeholder"
                         :disabled="scope.row[column.disabled]==undefined?false:scope.row[column.disabled]"
                         @change="column.onchange?column.onchange(scope.row):false"
                         @visible-change="column.visibleChange?column.visibleChange(scope.row):false">
                <el-option v-for="(item,index) in column.source"
            ...
            <span v-else-if="column.type=='select_edit_no_create2'"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                         size="small"
                         filterable
            ...
            <span style="width:100%;"
                  v-else-if="column.type=='select_edit'"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                         size="small"
                         allow-create
            ...
            <span v-else-if="column.type=='select_remote'"
                  :row="scope.row">
              <el-select v-model="scope.row[column.prop]"
                         size="small"
                         filterable
            ...
            <span v-else-if="column.type=='text'"
              ...
              <el-input v-model="scope.row[column.prop]"
                ...
                <template slot="append"
                          v-if="column.suffix&&column.showSuffixMethod && column.showSuffixMethod(scope.row)">%</template>
              </el-input>
              <el-input v-model="scope.row[column.prop]"
                        v-else
                        clearable
              ...
              <div v-if="scope.row.isToolTip && column.prop== 'paramAliasName' "
            ...
            <span v-else-if="column.type=='text_dynamic'">
              <el-input v-model="scope.row[column.prop]"
                        ...
                        :placeholder="column.placeholder"
                        size="small"
                        :disabled="scope.row[column.disabled]==undefined?false:scope.row[column.disabled]"></el-input>
            </span>
            ...
            <span v-else-if="column.type=='text_row_calc'">
              <el-input v-model="scope.row[column.prop]"
                        :placeholder="column.placeholder"
                        size="small"
                        :disabled="scope.row[column.compareColunm]==column.compareValue"></el-input>
            </span>
            <span v-else-if="column.type=='cmd'">
              <span v-for="(cmd,index) in column.cmds"
                    :key="index">
                <el-button v-if="cmd.rel=='del'"
                           size="small"
                           :type="cmd.type"
                  ...
                  {{cmd.label}}</el-button>
                <el-button v-else-if="cmd.cmd"
                           :disabled="scope.row[cmd.disabled]==undefined?false:scope.row[cmd.disabled]"
                           size="small"
                           :type="cmd.type"
                           @click="cmd.cmd(scope.row)"
                           :class="cmd.class">{{cmd.label}}</el-button>
              </span>
            </span>
            <div v-else-if="column.type=='cmd_circle'"
                 style="text-align: center;padding-top: 3px">
              <span v-for="(cmd,index) in column.cmds"
                    :key="index">
<!--                <img v-if="cmd.rel=='add'"-->
<!--                     src="@/assets/images/dataFocus/welcome/icon/add.png"-->
<!--                     style="width: 20px;height: 20px;cursor:pointer"-->
<!--                     @click="addRow(scope.row)"-->
<!--                     :class="cmd.class"-->
<!--                     :disabled="scope.row[cmd.disabled]==undefined?false:scope.row[cmd.disabled]" />-->
<!--                <img v-if="cmd.rel=='del'&&scope.row[cmd.disabled]==undefined?true:scope.row[cmd.disabled]"-->
<!--                     src="@/assets/images/dataFocus/welcome/icon/reduce.png"-->
<!--                     style="width: 20px;height: 20px;cursor:pointer;margin-left: 10px;"-->
<!--                     @click="delRow(scope.row)"-->
<!--                     :class="cmd.class" />-->
                <!--<el-button v-if="cmd.rel=='add'" size="small"   @click="addRow(scope.row)"-->
                <!--:class="cmd.class"-->
                <!--:disabled="scope.row[cmd.disabled]==undefined?false:scope.row[cmd.disabled]" :icon="cmd.type" circle> </el-button>-->
                <!--<el-button v-else-if="cmd.rel=='del'" size="small"   @click="delRow(scope.row)"-->
                <!--:class="cmd.class"-->
                <!--:disabled="scope.row[cmd.disabled]==undefined?false:scope.row[cmd.disabled]" :icon="cmd.type" circle> </el-button>-->
              </span>
            </div>
            <!--yangjing235 add at 2020-01-14 新增/删除事件调用cmd配置的事件-->
            <div v-else-if="column.type=='cmd_circle_cmdEvent'"
                 style="text-align: center;padding-top: 3px">
              <span v-for="(cmd,index) in column.cmds"
                    :key="index">
<!--                <img v-if="cmd.rel=='add'"-->
...
<!--                     :class="cmd.class">-->
            ...
            <span v-else-if="column.type=='cascader'">
              <div class="block">
                <el-cascader expand-trigger="hover"
                             ...
                             @change="column.onchange"></el-cascader>
                <el-cascader expand-trigger="hover"
                             clearable
                             size="small"
                             :placeholder="column.placeholder"
            ...
            <span v-else-if="column.type=='checkbox'">
              <el-checkbox v-model="scope.row[column.prop]"
                           ...
                           :disabled="scope.row[column.disabled]==undefined?false:scope.row[column.disabled]"
                           v-if="column.onchange"
                           @change="column.onchange(scope.row)"></el-checkbox>
              <el-checkbox v-model="scope.row[column.prop]"
                           size="small"
                           :disabled="scope.row[column.disabled]==undefined?false:scope.row[column.disabled]"
                           v-else></el-checkbox>

              <el-tooltip v-if="column.btn!==undefined&&column.btn.checkMergeStatus!==undefined&&column.btn.checkMergeStatus(scope.row)"
                          placement="right"
                          effect="light"
                          style="width: 100%">
                <div slot="content">
                  <el-table :data="column.btn.showInfo(scope.row)"
                            style="width: 100%"
                            max-height="400"
                            border>
                    <el-table-column prop="paramName"
                                     label="参数名"></el-table-column>
                    <el-table-column prop="paramCnName"
                                     label="参数中文名"></el-table-column>
                ...
                <el-button v-if="column.btn!==undefined&&column.btn.checkMergeStatus!==undefined&&column.btn.checkMergeStatus(scope.row)"
                           size="small"
      ...
      <!-- <span v-if="isPaginations">
        <div class="block">
          <el-pagination @size-change="handleSizeChange" @current-change="handleCurrentChange"
            :current-page.sync="currpag" :page-sizes="this.pageSizes" :page-size="pageSize" :total="tableData.length"
...
export default {
  ...
  data () {
    return {
      ...
      showsEditEtableData: [],
      showEditEtableData: [],
      isPaginations: this.isPagination,
      pageSize: 5,
      total: 0,
      currpag: 1,
    };
  ...
  computed: {
    tableData1: function () {
      ...
      //   this.tableData2 = this.tableData.where(0, "delStatus")
      //   this.showEditEtableData = this.getabpag(this.tableData2)
      //   return this.showEditEtableData;
      // } else {
  ...
  methods: {
    ...
    addRow () {
      ...
      // if (isPagination) {
      //   // this.currpag = this.lastPage
      //   this.getabpag()
      // }
      this.$emit("addRowData", rowCopy);
      // console.log(this.tableData.length / this.pageSize, 'this.lastPage')
      // this.currpag = this.lastPage
    ...
    upLoadChange (content) {
      ...
      //   if (!$.isEmptyObject(reader.result) && relArr.length > 1) {
      //     for (var key = 1, len = relArr.length; key < len; key++) {
      //       var values = relArr[key];
      //       var reg = new RegExp(" ", "g");//g,表示全部替换。
      //       values = (values == undefined) ? "" : values.replace(reg, "");
      //       if (values != '') {
      //         var obj = {};
      //         var objArr = values.split(",");
      //         /*if (this.columns.length != objArr.length) {
      //           alert("第" + key + "行数据列数错误!");
      //           return false;
      //         } else {*/
      //         var maxIndex = _this.columns.length < objArr.length ? _this.columns.length : objArr.length;
      //         $.each(_this.columns, function (i, column) {
      //           if (i >= maxIndex) return false;
      //           var regex = new RegExp("^[0-9]$");
      //           if (regex.test(objArr[i])) {
      //             obj[column.prop] = parseInt(objArr[i]);
      //           } else {
      //             obj[column.prop] = objArr[i];
      //           }
      //         })
      //         obj['insert'] = true;
      //         _this.tableData.push(obj);
      ...
      //   }
      //   console.log(data);
      //   //_this.tableData=$.extend(true,[],_this.tableData,data);
      // };
    },
    ...
    //   this.pageSize = val;
    //   // console.log(this.pageSize, "每页");
    //   this.getabpag(this.tableData2)
    // },
    // handleCurrentChange (val) {
    //   this.currpag = val;

    //   // console.log(this.currpag, '当前页')
    //   this.getabpag();
    // },
    // getabpag (item) {
    //   let list1 = []
    ...
    //       list2.push(list1)
    //     }
    //     this.showsEditEtableData = list2
    //     list1 = []
    //     list2 = []
    //   }

    //   return this.showsEditEtableData[this.currpag - 1]
    // },
  },
  mounted () {
    if (
