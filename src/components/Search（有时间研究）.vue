  ...
  <el-form :model="params"
    ...
    <el-form-item v-for="(form, index) in forms"
      ...
      <el-input v-if="form.itemType === 'input' || form.itemType === undefined"
                ...
                clearable />
      <el-autocomplete v-if="form.itemType === 'autocomplete'"
                       v-model="params[form.modelValue]"
                       :placeholder="form.placeholder"
                       :autofocus="form.autofocus"
                       @keyup.enter.native="searchHandler"
                       ...
                       @select="item=>handleSelect(item,form.prop)"></el-autocomplete>
      <el-checkbox v-if="form.itemType === 'checkbox'"
                   ...
                   :suffix-icon="form.suffixIcon"
                   :prefix-icon="form.prefixIcon"
                   :style="itemStyle + (form.itemWidth ? `width: ${form.itemWidth}px;` : '')">{{form.placeholder}}</el-checkbox>
      <el-select v-else-if="form.itemType === 'select'"
        ...
        <el-option v-for="(op, opIndex) in selectOptions[selectOptionPrefix + index]"
                   :key="opIndex + '_remote'"
                   :value="(typeof op === 'object') ? op[form.valueKey || 'value'] : op"
                   :label="(typeof op === 'object') ? op[form.labelKey || 'label'] : op" />
      </el-select>
      <el-select v-else-if="form.itemType === 'select2-1'"
                 :placeholder="form.placeholder"
                 :style="itemStyle + (form.itemWidth ? `width: ${form.itemWidth}px;` : '')"
                 :size="form.size ? form.size : size"
                 filterable
      ...
      <el-select v-else-if="form.itemType === 'select2-2'"
                 filterable
                 v-model="params[form.modelValue]"
                 :size="form.size ? form.size : size"
                 :disabled="form.disabled"
      ...
      <el-radio-group v-else-if="form.itemType === 'radio-button'"
                      v-model="params[form.modelValue]"
                      :disabled="form.disabled"
                      @change="searchHandler">
        <el-radio-button v-for="(option, optionIndex) in form.options"
                         :key="optionIndex"
                         :label="option.value">
      ...
      <span v-else-if="form.itemType=='cascader'">
        <div class="block">
          <el-cascader expand-trigger="hover"
                       ...
                       :style="itemStyle + (form.itemWidth ? `width: ${form.itemWidth}px;` : '')"></el-cascader>
        </div>
      </span>
    </el-form-item>
    <el-form-item label
      ...
      <el-button type="primary"
                 :size="size"
                 @click="searchHandler"
                 :loading="submitLoading">{{ submitBtnText }}
      </el-button>
      <el-button type="primary"
                 :plain="true"
                 :size="size"
                 v-if="showResetBtn"
                 @click="resetForm"
                 :loading="submitLoading">{{ resetBtnText }}</el-button>
    </el-form-item>
  </el-form>
</template>
...
export default {
  ...
  methods: {
    ...
    async getRemoteData (url, param, listField, resultHandler, dataKey) {
      const _this = this
      const res = await this.jaxios(url, param)
      let result = res
  ...
  mounted() {
    // this.params.likeKey == "" && this.$route.params.id ? this.params.likeKey = this.$route.params.id : this.params.likeKey;
    this.params.likeKey = this.$route.params.id === 'leftNav' ? '' : this.$route.params.id;
  }
}
</script>
